...
ggplot(fish_per_frame, aes(frame, n)) +
...
cv_per_frame %>% mutate(frame = as.numeric(frame)) %>%
  pivot_longer(cols = c(cv_pop, cv_ind, ratio), names_to = "Type", values_to = "CV") %>%
ggplot(aes(frame, CV))+
  geom_line(aes(col = Type))+
  geom_line(aes(frame, op, alpha = n_fish), col = "grey")+
  geom_line(aes(frame, or,alpha = n_fish), col = "darkgoldenrod")+
  theme_bw()

# op > 0.65 & or < 0.35 ~ "polarized",
# op < 0.35 & or > 0.65 ~ "milling",
