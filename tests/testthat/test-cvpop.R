test_that("cvpop returns a dataframe OR numeric value with the correct values", {
  ...
  result1 <- cvpop(df[,c("x", "y")], log10 = T) # if not accounting for yday
  result2 <- df %>% group_by(random) %>% summarise(pop = cvpop(across(c(x,y)), log10 = T))
  expect_type(result1, "double")
  expect_true(!is.na(result1))
  expect_equal(result1, result2$pop)

  result3 <- cvpop(cbind(lon, lat))
